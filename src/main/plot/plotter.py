...
def merge_samples(contents, configs):
  ...
  for config,sconfigs in list(configs.items()):
    ...
    time = [round(x) for x in matrices[0][0]] # time should be the same for all
    matrices = [l[1:] for l in matrices] # skips the time dimension for each sample
    ...
    merged = [x/nsamples for x in merged]
...
def plot(config,content):
  ...
  for nf, pformat in enumerate(the_plots_formats):
    ...
    if nf in forceLimitPlotX: axes.set_xlim(xmax = forceLimitPlotX[nf], xmin = startPlotX[nf])
...
with open(plotconfig, 'r') as stream:
    try:
        ...
        startPlotX = parse_sim_option(pc, 'start_plot_x', 0)
...
if sampling:
  ...
  for title,content in list(merged_contents.items()):
    plot(title,content)
else:
    for title,content in list(contents.items()): plot(title,content)
